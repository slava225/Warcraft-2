...
const scenes = {
    intro: {
        ...
        title: 'Тёплая кофейня у парка',
        text: ({ playerName }) => `${playerName} решает искать девушку бережно и без суеты. Вечером ты заходишь в небольшую кофейню, где пахнет карамелью и дождём. За стойкой девушка поправляет книгу и улыбается.

«Привет. Ты здесь впервые?»`,
        hint: 'Выбери тон общения. Он влияет на открытость и взаимность.',
        choices: [
            {
    ...
    direct: {
        ...
        hint: 'Смелость добавляет открытости, но важно оставаться бережным.',
        choices: [
            {
    ...
    playful: {
        ...
        hint: 'Лёгкость работает, если подпитывать её вниманием к собеседнице.',
        choices: [
            {
    ...
    book: {
        ...
        hint: 'Тонкие вопросы повышают взаимность. Можно поделиться личным опытом.',
        choices: [
            {
                text: 'Признаться, что ты тоже записываешь мысли, когда волнуешься.',
                next: 'shared',
                effects: { sincerity: 9, affection: 7, flags: { sharedHobby: true } }
            },
            {
                text: 'Сказать, что любишь письма и предлагаешь обменяться небольшими посланиями.',
                next: 'letters',
                effects: { affection: 8, flags: { likesLetters: true } }
            },
            {
    ...
    music: {
        ...
        hint: 'Совместный ритуал создаёт близость. Это повышает взаимность.',
        choices: [
            {
            ...
            },
            {
                ...
                next: 'quiet',
                effects: { sincerity: 6, flags: { valuesCare: true } }
            },
            {
    ...
    honest: {
        ...
        hint: 'Искренность повышает взаимность, но не забывай о лёгкости.',
        choices: [
            {
    ...
    shared: {
        ...
        hint: 'Отмечай общие ценности. Они усилят взаимность.',
    ...
    letters: {
        ...
        title: 'Тёплые письма',
        text: () => `Она достаёт маленькую открытку: «Тепло — это когда тебя слушают до конца». «Это про сегодня», — улыбается она.`,
        hint: 'Письма усиливают романтичность. Поддержи этот настрой.',
        notes: ['Любит письма'],
        choices: [
            {
                text: 'Ответить своим небольшим посланием и поблагодарить её.',
                next: 'final',
                effects: { affection: 8, sincerity: 4 }
            },
            {
                text: 'Предложить устроить совместный вечер писем.',
                next: 'final',
                effects: { confidence: 5 }
            }
        ]
    ...
    dream: {
        ...
        hint: 'Совпадение мечт добавляет взаимность.',
        notes: ['Общие мечты'],
        choices: [
            {
    ...
    final: {
        ...
        text: ({ playerName, affection, sincerity, confidence, flags }) => {
            ...
            const bonus = flags.likesLetters
                ? ' Она обещает прислать тебе маленькое письмо завтра.'
                : flags.valuesCare
                    ? ' Она говорит, что ей нравится твоя бережная энергия.'
                    : flags.sharedHobby
...
const saveGame = () => {
    localStorage.setItem('warm-search-state', JSON.stringify(state));
};

const loadGame = () => {
    const saved = localStorage.getItem('warm-search-state');
    if (saved) {
...
const init = () => {
    ...
    elements.restartBtn.addEventListener('click', () => {
        ...
        localStorage.removeItem('warm-search-state');
